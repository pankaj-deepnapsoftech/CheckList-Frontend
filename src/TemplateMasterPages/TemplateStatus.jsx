...
function WorkflowStatusViewModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-3xl max-h-[92vh] rounded-2xl border border-white/20 bg-white/80 backdrop-blur-xl shadow-2xl flex flex-col overflow-hidden">
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-200/50 bg-white/90 backdrop-blur-md px-6 py-4">
          <div>
            <h2 className="text-xl font-bold text-gray-900 tracking-tight">Workflow Status</h2>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {chain.map((item, index) => (
                <React.Fragment key={item.stage_index ?? index}>
                  {index > 0 && <span className="text-gray-400 font-medium select-none">→</span>}
                  <div
                    className={`inline-flex flex-col gap-1 rounded-xl border border-gray-200/60 bg-white shadow-sm px-4 py-3 min-w-[120px] ${item.type === "HOD" ? "ring-1 ring-blue-200/60" : ""}`}
                  >
                    <span className={`text-xs font-medium uppercase tracking-wider ${item.type === "HOD" ? "text-blue-600" : "text-gray-600"}`}>
                      {item.type === "HOD" ? "HOD" : "User"}
                    </span>
                    <span className="font-semibold text-gray-900 truncate max-w-[160px]" title={item.label}>
                      {item.label || "—"}
                    </span>
                    <span className={`inline-flex w-fit rounded-lg px-2 py-1 text-xs font-medium ${getStatusBadge(item.status)}`}>
                      {getWorkflowStatusLabel(item.status)}
                    </span>
                    {item.approved_at && (
                      <span className="text-xs text-gray-500">{new Date(item.approved_at).toLocaleString()}</span>
                    )}
                    {item.remarks && (
...
export default function TemplateStatus() {
  ...
  const { templateStatusListQuery, workflowStatusQuery } = useTemplateMaster(
    "",
    selectedRow != null ? selectedRow.template_id : null,
    selectedRow != null ? selectedRow.user_id : null
  );
          ...
          {templateStatusListQuery.isLoading ? (
            ...
            <div className="overflow-x-auto">
              ...
              <WorkflowStatusViewModal
                ...
                templateName={selectedRow != null ? selectedRow.template_name : null}
                workflowName={workflowData != null ? workflowData.workflow_name : null}
                chain={workflowData != null ? workflowData.chain : null}
                isLoading={workflowStatusQuery.isLoading}
                isError={workflowStatusQuery.isError}
                errorMessage={
                  workflowStatusQuery.error != null
                    ? (workflowStatusQuery.error.response && workflowStatusQuery.error.response.data && workflowStatusQuery.error.response.data.message) ||
